			...
			if (trim ( $keyword ) != '') {
				...
				if (count ( $res ) > 0) {
					...
					if(in_array('OPT_CJ_FULL', $camp_opt)){
						...
						echo '<br>Loading full page to find a description';
						...
						if(stristr($exec, 'advertise_compact') && stristr($exec,'<span class="source')  ){
							preg_match('{(<div class="advertise_compact">.*?)<p><span class="source}s' , $exec , $des_matches);
							...
							echo '<-- Can not find class advertise_compact, using summary instead';
						}
