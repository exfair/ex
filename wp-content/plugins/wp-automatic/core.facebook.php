			...
			if (  (stristr($exec, 'notifications.php')  && stristr($exec, $cg_fb_page_id)) || ( stristr($exec, 'notifications.php') && $cg_fb_from == 'events' )  ){ // Checks that the object is created correctly
				...
				foreach ( $items as $item ){
					...
					if(    ! stristr($item, '</p></span>') && trim($txtContent) != '' && ! $is_colored_post ){
						...
						*/
						...
						if(count($full_text_matches) > 2){
							$full_text_matches = array( $full_text_matches[0] , $full_text_matches[1] );
						}
						...
						if(trim($possible_full) != ''){
							...
							if(! in_array('OPT_FB_TXT_SKIP', $camp_opt)) $content =  $possible_full; 
						}
					...
					if($type == 'link' ){
						...
						if(in_array('OPT_GENERATE_NO_LINK',$camp_opt)) $title = '';
...
<div class="fb-post" data-href="https://www.facebook.com/'.$cg_fb_page_id.'/posts/'. $single_id .'"></div>';
					...
					if(in_array('OPT_FB_COMMENT', $camp_opt)){
						...
						if( stristr($exec2_raw, '{"comments"' ) || stristr($exec2_raw, '{comments:' )  ){
									...
									if( isset( $comments_json->comments ) && count( $comments_json->comments  ) > 0 ){
										...
										foreach($comments_json->comments as $comment_obj ){
											...
											if ( isset($comment_name_matches[1] ) && trim($comment_name_matches[1]) != '' ){
												...
												if(isset($commenter_name_json[0])) $commenter_name = $commenter_name_json[0];
							...
							//edges:[{node ...__typename:"Comment"},cursor:"AQHRJpdJxCiEwoRdtdydeisnvpjtTxJ9wk1_OAMIFGwB5ylEL1Wl2fidayVd8mErMcU2Sul_ftHtFzFoCRI_3cFGSg"}]
							preg_match_all( '{edges:\[\{node[^<]*__typename:"Comment"\},cursor:".*?"\}\]}s', $exec2_raw , $comment_part_matches);
							...
							if( isset($comment_part_matches[0]) && trim( $comment_part_matches[0] ) != '' ){
								...
								foreach ($comment_part_matches as $part_key => $part_value){
									...
									if(! stristr($part_value, $single_id)) unset( $comment_part_matches[$part_key] );
									
								}
								
								if(count($comment_part_matches) > 0) $comment_part_matches = array_values($comment_part_matches);
							...
							if( isset($comment_part_matches[0]) && trim( $comment_part_matches[0] ) != '' ){
								...
								if(isset($comments_json->edges) ){
									...
									if(count($all_the_comments) >0) $ret['comments'] = array_reverse( $all_the_comments );
