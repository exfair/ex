		...
		class wp_automatic {
			...
			function process_campaign($camp) {
				...
				if (trim ( $title ) != '') {
					...
					if (in_array ( 'OPT_TRANSLATE', $camp_opt ) && trim ( $abcont ) != '' && $camp->camp_translate_from != ''  && $camp->camp_translate_from != 'no'   && $camp->camp_translate_to != ''  && $camp->camp_translate_to != 'no' ) {
					...
					if (in_array ( 'OPT_CACHE', $camp_opt ) || $camp_type == 'Instagram'  || $camp_type == 'Clickbank') {
						...
						foreach ( $srcs as $image_url ) {
							...
							if(in_array('OPT_CACHE_CLEAN' , $camp_opt   )){
								...
								$clean_name = '';
								$clean_name =  sanitize_file_name( remove_accents( wp_trim_words( $post_title ,10 , '' ) ) );
					...
					if(in_array('OPT_KEYWORD_CAT', $camp_opt) && trim($camp_general['cg_keyword_cat']) != '' ){
						...
						foreach ($cg_keyword_cat_rules as $cg_keyword_cat_rule) {
							if(stristr( $cg_keyword_cat_rule , '|')){
								...
								if( preg_match('{\b' . preg_quote( $cg_keyword_cat_rule_keyword ). '\b}siu', $content_to_check) || (stristr($cg_keyword_cat_rule_keyword, '#') && stristr($content_to_check, trim($cg_keyword_cat_rule_keyword))  )){
						...
						if(count($customPostTaxonomies) > 0)
						{
							...
							foreach($customPostTaxonomies as $tax)
							{
								...
								if(is_taxonomy_hierarchical($tax)){
									if( trim($camp->camp_post_category) != '' ) echo '<br>Setting taxonomy '.$tax.' to '.$camp->camp_post_category;
					...
					if ($camp_type == 'SoundCloud') {
						...
						if(in_array('OPT_SC_COMMENT', $camp_opt)){
							...
							$wp_automatic_sc_client= 'YUKXoArFcqrlQn9tfNHvvyfnDISj04zk';
									...
									if( trim($comment['text']) != '' ){
										...
										if(trim($commentText) != '' ){
											...
											if($camp->camp_post_type == 'topic' && function_exists('bbp_insert_reply') ){
												...
												$reply_data = array('post_parent'=>$post_parent, 'post_content' =>$commentText , 'post_author' => false ,'post_date' => $time );
												$reply_meta = array( 'topic_id' => $post_topic_id , 'anonymous_name' =>$commentAuthor , 'anonymous_email' => $anonymous_email , 'anonymous_website' => $commentUri );
												$ret = bbp_insert_reply($reply_data,$reply_meta);
					...
					if($camp_type == 'Amazon' && $camp->camp_post_type == 'product'){
						...
						$camp_post_custom_k = array_merge ($camp_post_custom_k , array('product_price_updated','product_asin','product_price','product_list_price','_regular_price','_price','_sale_price','_visibility', '_product_url','_button_text','_product_type'));
						
						$wp_automatic_woo_buy = get_option('wp_automatic_woo_buy','Buy Now');
						if(trim($wp_automatic_woo_buy) == '') $wp_automatic_woo_buy = 'Buy Now' ;
						
						$camp_post_custom_v = array_merge ( $camp_post_custom_v , array( $now , '[product_asin]','[product_price]','[product_list_price]','[list_price_numeric]','[price_numeric]','[price_numeric]','visible','[product_link]',$wp_automatic_woo_buy,'external'));
							...
							if (  trim ($key) != ''   ) {
								...
								if($key == 'excerpt'){
									...
									if( ($camp_type == 'Feeds' || $camp_type == 'Single'  || $camp_type == 'Multi') &&   (trim($key) == '_price' ||  trim($key) == '_sale_price'   ||  trim($key) == '_regular_price') ) {
										...
										if(trim($possible_price) != '') $key_val = $possible_price ;
			   ...
			   // Concat title and content in one text
				...
				if( $translationMethod == 'googleTranslator' ){
					...
					try {
						...
						//fix html entities
						if(stristr($translated, ';')) $translated = htmlspecialchars_decode($translated , ENT_QUOTES );
				...
				for ($i=0;$i<$max_redir;$i++){
					...
					if(stristr($exec, 'http-equiv="refresh"') && $info['http_code'] == 200 && ! stristr($exec, '_fb_noscript')){
						...
						preg_match('{location\.replace\((.*?)\)}' , $exec , $loc_matches); 
							...
							$possible_redirect = $loc_matches[1];
							
							if(stristr($possible_redirect, 'http' )){
								
								$possible_redirect = str_replace(array("'",'"'), '', $possible_redirect);
...
$agents = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_6) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/12.1.2 Safari/605.1.15
...
return trim($arrAgents[$rand]);
			...
			function lazy_loading_auto_fix($cont){
				...
				if ($imgs_count < 1)
					...
					if (false && stristr ( $original_cont, ' data-src' )) {
						$found_lazy_tag = 'data-src';
					} elseif (false && stristr ( $original_cont, ' data-src' )) {
						...
						if (stristr ( $images_plain, 'lazy' )) {
							...
							// src values
							preg_match_all ( '{ src="(.*?)"}', $images_plain, $srcs_matches );
					...
					if(trim($found_lazy_tag) != ''){
						echo '<br>Lazy loading auto-detected lazy tag: '.$found_lazy_tag . ' fixing lazy loading....';
				...
				if(! stristr($cont,$cg_feed_lazy)) return $cont;
