		...
		if (trim ( $keyword ) != '') {
			...
			if (count ( $res ) > 0) {
				...
				if(in_array('OPT_YT_TAG', $camp_opt)){
					...
					if(  (stristr($exec, 'name="keywords"')  || stristr($exec,'keywords":"') || stristr($exec, 'keywords":['  ) ) && ! stristr($exec, 'recaptcha')   ){
					...
					}elseif( stristr($exec, 'recaptcha') ){
						echo '<--recaptcha found will not be able to get tags';
					}else{
