    ...
    jQuery('#campaign_run').click(function(){
    	...
    	jQuery.ajax({
    	    ...
    	    ,error: function(one, two , three){
    	     	...
    	     	jQuery('#campaign_run').addClass('run').removeClass('run_load');
	    	    	...
	    	    	jQuery('#welcome-panel').remove();
	    	    	jQuery('#status-meta-boxes .inside').append('<div style="display:none;margin-top:0" dir="ltr" class="welcome-panel" id="welcome-panel"> '+  two + ' ' +  three +' <a dir="rtl" style="right:10px !important" href="#" class="welcome-panel-close">Dismiss</a><div class="welcome-panel-content"></div></div>');
	    	    	jQuery('#welcome-panel').slideDown('slow',function(){
		...
		(function($){
			...
			$('.TTWForm').on('click','[data-popup-field-name]', function(e){
				...
				// Encoding
				if(jQuery('input[value="OPT_FEED_ENCODING"]').attr('checked') == 'checked' ){
						  ...
						  if(jQuery(this).hasClass('highlight')){
							  ...
							  if( jQuery('#insepector_frame').attr('src').indexOf('cg_ml_lnk_visual') != -1 ){
								  ...
								  if ( jQuery( foundElement).attr('href') != undefined ){
									  ...
									  while ( currentParent != undefined && jQuery(currentParent).attr('href') == undefined  ){
