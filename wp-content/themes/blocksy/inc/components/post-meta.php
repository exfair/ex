	...
	function blocksy_post_meta($post_meta_descriptor = null, $args = []) {
		...
		foreach ($post_meta_descriptor as $single_meta) {
			...
			if (
				...
				&&
				get_the_author()
			) { ?><li class="meta-author" <?php blocksy_schema_org_definitions_e('author_name') ?>>
					<?php if ($single_meta['has_author_avatar'] === 'yes') {
				...
				if ($single_meta['id'] === 'post_date') {
					?><li class="meta-date" <?php blocksy_schema_org_definitions_e('publish_date') ?>>
			...
			if ($single_meta['id'] === 'updated_date') {
				?><li class="meta-updated-date" <?php blocksy_schema_org_definitions_e('modified_date') ?>>
					<?php if ($args['meta_type'] === 'icons' || $args['force_icons']) { ?>
						<svg width="13" height="13" viewBox="0 0 15 15">
