...
function blocksy_related_posts($location, $is_cache_phase = false) {
	global $post;
	...
	$label_tag = get_theme_mod('related_label_wrapper', 'h3');

	$container_class = 'ct-container';

	if (get_theme_mod('related_structure', 'normal') === 'narrow') {
	...
	<?php } ?>
			...
			<div class="ct-related-posts" data-columns="<?php echo esc_attr($columns); ?>">
				...
				<?php while ($query->have_posts()) { ?>
					...
					<article <?php blocksy_schema_org_definitions_e('creative_work') ?>>
						<?php
